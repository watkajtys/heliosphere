...
/**
 * Applies a circular crop and feather to an image.
 * @param imageBuffer The image to process.
 * @param cropRadius The radius of the circular crop.
 * @param featherRadius The radius of the feather effect.
 * @returns A promise that resolves to the processed image buffer.
 */
async function applyCircularFeather(imageBuffer: Buffer, cropRadius: number, featherRadius: number): Promise<Buffer> {
	const size = cropRadius * 2;

	// First, resize the image to the crop diameter.
	...
	const featherStartRatio = (cropRadius - featherRadius) / cropRadius;
	const svgMask = `
 ...
 * @param cropRadius The radius to crop the sun disk to.
 ...
 */
async function createTunedCompositeImage(isoDate: string, apiKey: string, cropRadius: number, featherRadius: number): Promise<Buffer> {
	const width = 1920;
	const height = 1200;
	...
	const featheredSunDisk = await applyCircularFeather(sunDiskImage, cropRadius, featherRadius);

	// Determine the final canvas size
	const finalWidth = Math.max(width, cropRadius * 2);
	const finalHeight = Math.max(height, cropRadius * 2);
...
app.get('/composite-image', async (req, res) => {
	...
	const cropRadius = req.query.cropRadius ? parseInt(req.query.cropRadius as string) : 1154;
	const featherRadius = req.query.featherRadius ? parseInt(req.query.featherRadius as string) : 40;

	try {
		const imageBuffer = await createTunedCompositeImage(isoDate, apiKey, cropRadius, featherRadius);
...
// Redirect root to the tuning page
app.get('/', (req, res) => {
	res.redirect('/tune');
});
