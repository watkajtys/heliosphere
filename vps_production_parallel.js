...
async function gradeSunDisk(imageBuffer) {
    return await sharp(imageBuffer)
        .modulate({ saturation: 1.2, brightness: 1.2, hue: 15 })
...
async function processDateRange() {
    ...
    const fetchQueue = frames.map((date, i) => ({ number: i, date }));
    ...
    async function fetchNextFrame() {
        ...
        const frameKey = getFrameKey(date);
        const framePath = getFramePath(date);
        ...
        try {
            ...
            const [coronaData, sunDiskData] = await Promise.all([
                fetchImageWithFallback(date, 4, 'SOHO'),
                fetchImageWithFallback(date, 10, 'SDO')
            ]);
            ...
            console.error(`❌ Failed frame ${number}: ${error.message}`);
            missingFrames.push(number);
